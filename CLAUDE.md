...
Requires `ANTHROPIC_API_KEY` in a `.env` file for the AI analysis feature. All other features work without it.
...
ui/                            # All PyQt6 UI components
    ...
    ai_analysis_dialog.py      # AIAnalysisDialog (QDialog) — score gauge, pros/cons, cached timestamp
...
core/                          # Business logic, data, background workers
    ...
    ai_analysis_worker.py      # AIAnalysisWorker (QThread) — fetches Senate data + calls Claude API
    senate_worker.py           # SenateWorker (QThread) — fetches Senate trades from senatestockwatcher.com
...
6. AI analysis runs in `AIAnalysisWorker` (QThread) — fetches Senate trades then calls Claude API
7. Senate trades panel is populated by `SenateWorker` (QThread) on every stock load
...
- **No table view** — the left panel shows a stock info panel (symbol, name, current price, day change, prediction results, AI score, Senate trades).
...
3. AI Analysis section — button, score (+N / -N), description (Bullish / Bearish etc.)
4. Senate Trades section — scrollable list of recent senator trades for the selected stock
...
- `AIAnalysisWorker` fetches up to 15 recent Senate trades for the symbol from `senatestockwatcher.com`
- Builds a 30-day price summary (current price, change %, high, low, avg volume)
- Sends both to Claude (`claude-sonnet-4-6`) with a structured prompt requesting a JSON response: `{score, pros, cons}`
...
- The dialog (`AIAnalysisDialog`) shows the score as a large colored number, a center-zero bar, and two-column pros/cons list; displays "Cached · <timestamp>" when showing a saved result
...
### Senate Trades (senatestockwatcher.com)
- `SenateWorker` fires on every stock load (not just AI analysis)
- Fetches up to 20 recent trades for the symbol
- Displayed in a scrollable section at the bottom of the left panel
- Color-coded: ▲ BUY in green, ▼ SELL in red
- Failures are handled silently — shows "No recent Senate trades found"
...
`dfpath` is **filename only** — always join with `csv_path` to get the full path. `ai_analysis` is optional; absent if the user has never run analysis for that stock.
